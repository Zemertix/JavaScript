 ...
 let f1WallArray = [[-10, 0], [14, 32], [42, 53], [64, 74], [92, 105], [119, 120]];
 ...
 let heroImg = window.document.querySelector('#hero-image');
...
let heroY = Number.parseInt(imgBlock.style.bottom)/32;
 ...
 jumpBlock.style.top = `${window.screen.height/2 - 130}px`
 hitBlock.style.top = `${window.screen.height/2 - 130}px`
...
let onTouchStart = (event) =>{
    clearInterval(timer);
    x = (event.type === 'mousedown') ? event.screenX : x = event.touches[0].screenX;
    timer = setInterval(() =>{
        if(x > halfWidth){
             rightHandler()
             direction ='right'
...
let createTileBlack = (x, y = 0) =>{
    ...
    tileBlack.style.pointerEvents = 'none';
...
class Lever {
    ...
    constructor(){
        this.x = heroX - 20;
        this.y = heroY;
        this.fountainImg = objectArray.filter(elem => elem.outerHTML.split('"')[1] === 'img/assets/3 Objects/Fountain/2.png')[0]; 
...
class Terminal extends Cutscene {
    ...
    constructor() {
        ...
        this.btnBlock.style.bottom = `${10}vh`;
        this.btnBlock.style.width = '33%';
        this.btnBlock.style.height = '33%';
...
class Enemy1 extends Enemy {
    constructor(x,y,mess){
        super(x,y, 'img/assets/Enemies/1/', mess);
    }
...
class Enemy2 extends Enemy {
    constructor(x,y,mess, isLast){
        super(x,y, 'img/assets/Enemies/2/',mess,isLast)
    }
    setAttack(){
        this.img.src = this.sourcePath + 'Attack.png';
        this.spriteMaxPos = 5;
...
class Enemy5 extends Enemy {
    constructor(x,y,mess){
        super(x,y, 'img/assets/Enemies/5/', mess)
    }
    setAttack(){
        this.img.src = this.sourcePath + 'Attack.png';
        this.spriteMaxPos = 3;
...
class Enemy6 extends Enemy {
    ...
    bulletX;
    constructor(x, y,mess){
        super(x, y, 'img/assets/Enemies/6/', mess);
...
const updateHearts = () =>{
        ...
        for(let i = 0; i < lives; i++){
            heatsArray[i].img.style.display = 'block';
        }
        for(let i = lives; i < maxLives; i++){
            heatsArray[i].img.style.display = 'none';
        }
}
...
const createBackImg = (i) =>{
    ...
    img.style.width = `${window.screen.width + 120}px`;
...
const addDecorationElements = (f1, f2, f3) =>{
    ...
    // Tree
    createImgEl(basePath +'/Other/Tree4.png', 5, f1);
...
const addEnemies = () =>{
    ...
    let enemy5 = new Enemy1(79, 1, 'Третья цифра пароля это "2"');
    let enemy6 = new Enemy6(93, 5);
    let enemy7 = new Enemy2(100, 9, 'Последння цифра пароля это - "3"\n\nСкорее ищи рычаг, у тебя 15 секунд!', true);
}
